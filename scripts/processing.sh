...
find ../"$OUTPUT_DIR_SPLIT/" -type f -print0 | xargs -0 -P $NUM_PROC -I {} RNAfold -o {} -i {}

OUTPUT_DIR_CT=$FILE.ct
mkdir "../$OUTPUT_DIR_CT"
cd "../$OUTPUT_DIR_CT"
echo "Running b2ct on $OUTPUT_DIR_FOLD/* into $OUTPUT_DIR_CT"
find ../"$OUTPUT_DIR_FOLD/" -type f -name "*fold" -print0 | xargs -0 -P $NUM_PROC -I {} /bin/bash -c 'b2ct < "{}" > "$(basename {}).ct"'
...
find ../"$OUTPUT_DIR_CT/" -type f -print0 | xargs -0 -P $NUM_PROC -I {} /bin/bash -c 'perl /pyfasta/scripts/ct2bFrames.pl < "{}" > "$(basename {}).ss.b"'
...
find ../"$OUTPUT_DIR_CT/" -type f -print0 | xargs -0 -P $NUM_PROC -I {} /bin/bash -c 'perl /pyfasta/scripts/ct2bFrameStats.pl < "{}" > "$(basename {}).txt"'
